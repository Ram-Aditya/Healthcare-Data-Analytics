...
def get_labels(names, labels=None, label_file='data/trainLabels.csv', 
    ...
    labels=np.array([0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2])

    if per_patient:
        left = np.array(['left' in n for n in names])
        print (left)
        print(labels)
        print("***************labels ",labels[~left])
...
def split_indices(files, labels, test_size=0.1, random_state=RANDOM_STATE):
    ...
    print(type(labels))
    t=np.array([1,3])
    # labels=labels.append(1)
    # labels=labels.append(3)
    labels=np.array([0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2,0,3,2,1,0,2])
...
def split(files, labels, test_size=0.1, random_state=RANDOM_STATE):
    ...
    train=np.array([i for i in range(len(labels)-20)])
    test=np.array([i for i in range(len(labels)-20,len(labels))])
    print("Trin and teT: ",train,test)
